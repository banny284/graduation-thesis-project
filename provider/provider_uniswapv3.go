...
func NewUniswapV3Provider(
	...
	availablePairs, _ := provider.GetAvailableCurrencyPairs()
...
func (p *UniswapV3Provider) Poll() error {
	...
	for symbol, pair := range p.getAllPairs() {
		...
		sqrtx96 := util.StrToDec(fmt.Sprintf("%v", decoded[0]))
		...
		price := sqrtx96.Power(2).Quo(cosmosType.NewDec(2).Power(192))

		var diff uint64
		if decimalsBase >= decimalsQuote {
			diff = decimalsBase - decimalsQuote
			price = price.Mul(cosmosType.NewDec(10).Power(diff))
		} else {
			diff = decimalsQuote - decimalsBase
			price = price.Quo(cosmosType.NewDec(10).Power(diff))
		}
...
func (p *UniswapV3Provider) GetAvailableCurrencyPairs() (map[string]struct{}, error) {
